...
/**
 * Filter executed once per request to inspect Authorization header, extract JWT Bearer token,
 * validate authenticity, and populate the Spring SecurityContext.
 */
@Component
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (StringUtils.hasText(jwt) && jwtTokenProvider.validateToken(jwt)) {
                ...
                // Only ACCESS tokens are valid for invoking protected API endpoints
                if (JwtTokenProvider.TOKEN_TYPE_ACCESS.equalsIgnoreCase(tokenType)) {
                    ...
                    log.warn("Rejected non-access token type '{}' for request to {}", tokenType, request.getRequestURI());
                }
            }
        } catch (Exception ex) {
    ...
    /**
     * Extracts token from the HTTP Authorization header.
     */
